...
//#region node_modules/.nitro/vite/services/ssr/assets/auth-Cmxr6UUC.js
...
var login = createServerFn({ method: "POST" }).inputValidator((d) => {
	...
	const { verifyPassword, setSessionCookie } = await import("./auth-cookies.server-CaMv2Sys.mjs");
...
var logout = createServerFn({ method: "POST" }).handler(logout_createServerFn_handler, async () => {
	const { clearSessionCookie } = await import("./auth-cookies.server-CaMv2Sys.mjs");
	clearSessionCookie();
});
...
var requireAuth = createServerFn({ method: "GET" }).handler(requireAuth_createServerFn_handler, async () => {
	const { isSessionValid } = await import("./auth-cookies.server-CaMv2Sys.mjs");
